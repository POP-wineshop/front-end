...
const userSlice = createSlice({
  ...
  reducers: {
    // setUser: 사용자 정보를 상태에 저장하는 함수(로그인 성공 시 사용)
    setUser(state, action) {
      return action.payload; // action.payload에 담긴 사용자 정보로 상태를 교체
    },
    // clearUser: 사용자 정보를 초기화하는 함수(로그아웃 시 사용)
    clearUser() {
      return null; // 상태를 null로 초기화
    },
  },
});
