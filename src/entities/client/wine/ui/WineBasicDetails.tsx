...
const WineBasicDetails = ({ wineData }: { wineData: WineData }) => {
  const {
    korName,
    engName,
    price,
  ...
  const instantOrderData = createInstantOrderData(wineData, itemQuantity);
  const cartItemData = createCartItemData(wineData, itemQuantity);
    ...
    <div className="wine-description-top flex flex-col justify-center items-center lg:flex-row px-8 py-10 bg-[#f7f4f1]">
      ...
      <div className="wine-description-top-right font-pretendard w-[500px] h-[600px]">
        ...
        <table className="text-sm mb-6 w-full">
          <tbody>
            {getWineBasicDetails(wineData).map(({ key, value }) => (
              <tr
                key={key}
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center border rounded-md overflow-hidden">
              <button
                ...
                onClick={() => substractQuantity(itemQuantity, setItemQuantity)}
              >
                -
              </button>
              ...
              <button
                className="px-3 py-1 text-lg"
                onClick={() => addQuantity(itemQuantity, setItemQuantity)}
              >
                +
              </button>
            </div>
...
export default WineBasicDetails;
